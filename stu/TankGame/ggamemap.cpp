	...
	for(i=0; i<MAP_H; i++)
	{
		...
		for(j=0; j<MAP_W; j++)
		{
			...
			if (mask == 0xffff)
			{
				...
				mpTileBmp->draw(dc, x, y, TILE_W, TILE_H, sx, 0, false);
			}
			else
			{
				for(k=0; k<16; k++)
				{
					...
					{	
						sx = obj * TILE_W + k % 4 * 8;
						sy = k / 4 * 8;
						mpTileBmp->draw(dc, x + k%4*8, y + sy, 8, 8, sx, sy, false);
					}
					mask <<= 1;
				}
			}
...
int GGameMap::shootObj(const GBullet &bullet, bool isPowerful)
{
	...
	case DIR_DOWN:
		...
		if (!isPowerful)
			shift = (3 - (y % TILE_H) / 8) * 4;	
		else
			shift = (1 - (y % TILE_H) / 16) * 8;
		...
		if (x % TILE_W)		// 子弹仅在col列内
		{
			if (!isPowerful)
				...
				mask = 0xff << shift;
			if (mMap[row][col].mMask & mask)	// 如果子弹掩码与Tile掩码相交, 认为摧毁该Tile
			{
		...
		else				// 子弹正好位于col和col-1两个列中间
		{
			...
			mask >>= 2;
			if(col >= 1 && mMap[row][col-1].mMask & mask)
			{
				if (!canShoot(target))
					target = mMap[row][col-1].mObj;
				if (canDestroy(row, col-1, isPowerful))
					mMap[row][col-1].mMask &= ~mask;
			}
			if(mMap[row][col-1].mMask == 0)
